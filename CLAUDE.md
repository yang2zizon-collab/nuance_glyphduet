...
- **캐릭터 소개 컷신(스토리보드 기반 재제작)**: 룰렛 착지 750ms 후 재생, 디더링(도트)+컬러 레이어.
  사용자 스케치(IMG_0519)·스토리보드(IMG_0520) 기반.
  - tomato(8s): 토마토 덩굴밭(덩굴 커튼+매달린 토마토들), 핑크토마토에게 **서서히 줌인**(zoom 1→2.05),
    눈물 또르르, '…'
  - 심해어(1) 착지만 → phone(새 2 착지 시, 11s, 3비트): ①새가 나무 가지로 날아와 다이얼 전화기 발견(!)
    ②심해어도 어두운 심해 바위틈에서 폰 발견(!) ③위 하늘 새/아래 심해 심해어가 수화기 들고
    벨(호 확산)→'?'/'?'→'♪'. 비트 전환은 화이트 플래시.
  - mouse(8s): 부엌(싱크대·창문 빛), 부스러기 따라 쫄쫄쫄(먹으면 사라짐)→커다란 발 쿵+빗자루 스윽스윽
    (부스러기 쓸림)→줄행랑→왼쪽에서 빼꼼 '…'
  - 캐릭터만 컬러(cctx+characterColor), 배경 흑백 망점. 마지막(생쥐) 컷신 후 ▶ ready.
- **캐릭터 디자인(스케치 기반, sprites.js MINI_DRAW)**: 몸 검정 통짜 + 디테일 흰색 —
  토마토(울퉁불퉁 굴곡, 흰 별 꼭지+흰 눈·세모 입) / 심해어(아귀: 더듬이 발광체 깜박, 벌린 입에
  삐죽 이빨, 흰 하트 눈) / 새(서 있는 옆모습, 위로 벌린 부리, 흰 눈) / 생쥐(큰 귀+작은 귀,
  뾰족 코, 길게 휘는 꼬리, 흰 눈). silhouetteFill이 흰 픽셀(합 620↑)을 보존한다.
