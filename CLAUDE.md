...
- **캐릭터 소개 컷신(스토리보드 기반 재제작)**: 룰렛 착지 750ms 후 재생, 디더링(도트)+컬러 레이어.
  ...
  전 장면 공통 필름 비네트(카메라 변환 밖, 화면 고정).
  - tomato(12s, 2장면): A(0~5.2s) 잎 무성한 덩굴밭 — 결각·잎맥 잎(leaf), 광택 열매+별 꽃받침
    (tomatoFruit/calyx, 어두운 할로로 분리), 맨 위 안 익은 초록 하나, 핑토도 매달려 있음 →
    화이트 플래시 → B(5.2~12s) 나무 상자 — 슬랫·나뭇결·모서리 기둥, 무더기 속 핑토(미운오리새끼),
    줌인 후 눈물 또르르(8.2s~), '…'(10.2s)
  - 심해어(1) 착지만 → phone(새 2 착지 시, 11s, 3비트): ①새가 나무 가지로 날아와 다이얼 전화기 발견(!)
  ...
  - mouse(8s): 부엌(싱크대·창문 빛), 부스러기 따라 쫄쫄쫄(먹으면 사라짐)→커다란 발 쿵+빗자루 스윽스윽
    (부스러기 쓸림)→줄행랑→왼쪽에서 빼꼼 '…'. 발은 사진 레퍼런스 — 연한 청바지(외곽선·페이드 결·
    밑단 프레이)+흰 양말+어글리 스니커(밝은 바디·머드가드/힐카운터 패널·메쉬·검정 끈·러그 밑창).
...
- **캐릭터 디자인(sprites.js MINI_DRAW)**: 핑토(0)만 신규 — 동그란 몸 + 흰 별 꼭지 + 흰 눈·세모 입.
  ...
  전화 컷신 소품 rotary/handset은 현실적 비례(사다리꼴 몸체·크래들·구멍 10개 다이얼·컵 수화기).
