...
import { titleLowState, fadeMasterOut, restoreMaster, initAudio, typeBlip, typeKey, typeVoice, uiClick, speakVoiceEvents, playEnsemble, warmVoices, playScore,
         ...
         giftChime, startGiftAmbience, stopGiftAmbience, startEndingBed, stopEndingBed, playEndingMusic, onTitleGrain,
         startQrMusic, stopQrMusic, qrMusicSwell, startCosmicAmbience, stopCosmicAmbience,
  ...
  if (ln && fade > 0.2) {
    ...
    if (audioReady) {
      ...
      if (K.n > prev) {
        ...
        if (ch && ch !== ' ' && !'—…,.!?'.includes(ch)) typeKey(ch, 0.16, 4 + (li % 2), 0.4);   // 낭독은 BGM 아래로(음량 40%)
        sp[li] = K.n;
      }
    }
    sctx.save();
...
function startScreenAudio(name) {
  ...
  playScreenWav(name).then((played) => {
    ...
    else if (name === 'qr') startQrMusic();      // 관객 입장 대기 — 비컨 신호 음악
...
window.addEventListener('pointerdown', (e) => {
  ...
  const onMove = (ev) => {
    ...
    dragged = true;
    camDragYaw += (ev.movementX || 0) * 0.006;
    camDragPitch = Math.max(-1.2, Math.min(1.2, camDragPitch + (ev.movementY || 0) * 0.004));
  };
  const onUp = () => {
    window.removeEventListener('pointermove', onMove);
    window.removeEventListener('pointerup', onUp);
    if (!dragged) addAudienceNote();   // 안 움직였으면 탭 = 음표
  };
...
function giveSpecificGift(recip, giver) {
  ...
  floatGift(item);          // 배경에 그 아이템이 몇 개 떠다닌다(선물 단계 동안)
...
// 라운드마다 박자가 섞인다 — 4/4·3/4·6/8. level: 1=강박(큰 점) 0.6=중박 0=약박(작은 점).
// countIn = 예비박 길이(그 박자의 한 마디) — 예비박부터 그 박자로 센다.
const METERS = [
  { name: '4/4', len: 8, countIn: 4, level: (i) => (i % 4 === 0 ? 1 : i % 2 === 0 ? 0.6 : 0) },
  { name: '3/4', len: 6, countIn: 3, level: (i) => (i % 3 === 0 ? 1 : 0) },
  { name: '6/8', len: 6, countIn: 6, level: (i) => (i % 6 === 0 ? 1 : i % 6 === 3 ? 0.6 : 0) },
];
let roundMeter = METERS[0];      // 이번 라운드의 박자
let qrLeaving = false;           // QR→대화 전환 연출 중복 방지
...
function beginCountIn() {
  state.phase = 'countin';
  roundMeter = METERS[0];   // 4/4로 통일 (변박은 과했음 — METERS에 3/4·6/8 인프라는 남겨둠)
  resetBeatStep();                                    // 베이스 패턴을 마디 첫 박에 정렬
  beatsLeft = roundMeter.countIn;                     // 그 박자의 한 마디만큼(4/4→4, 3/4→3, 6/8→6)
  nextBeatAt = performance.now();                     // 첫 박 즉시
  phaseEndsAt = performance.now() + roundMeter.countIn * beatMs();
  if (hidden) hidden.blur();
  $('#round-gauge')?.classList.add('hidden');
  const lbl = $('#countin-label'); if (lbl) lbl.textContent = `${roundMeter.name} · ${Math.round(bpm)} BPM · ready`;
...
function beginRound() {
  state.phase = 'round';
  // 박자는 예비박(beginCountIn)에서 이미 정해졌다 — 예비박과 라운드가 같은 박자로 흐른다.
  beatsLeft = roundMeter.len;
  nextBeatAt = performance.now();                     // 라운드 첫 박 즉시
  phaseEndsAt = performance.now() + roundMeter.len * beatMs();
  $('#countin')?.classList.add('hidden');
  $('#round-gauge')?.classList.remove('hidden');
  const dl = $('#round-dur'); if (dl) dl.textContent = `${roundMeter.name} · ${Math.round(bpm)}BPM`;
  // 박 수만큼 점으로 리셋 — 강박은 큰 점, 중박은 중간, 약박은 작은 점.
  const dots = $('#beat-dots');
  if (dots) dots.innerHTML = Array.from({ length: roundMeter.len }, (_, i) => {
    const lv = roundMeter.level(i);
    return `<span class="beat-dot ${lv >= 1 ? 'strong' : lv > 0 ? 'mid' : 'weak'}"></span>`;
  }).join('');
...
function tickCycle() {
  ...
  while (beatsLeft > 0 && now >= nextBeatAt) {
    if (countin) {
      ...
      countTick(beatsLeft, roundIndex);                        // 카운트 틱 — 라운드마다 업그레이드
      // 예비박도 그 박자의 강약대로 — 강박엔 강한 킥, 중박엔 클랩(라운드와 같은 그루브로 진입).
      const cIdx = roundMeter.countIn - beatsLeft;
      const cLv = roundMeter.level(cIdx);
      beatKick(cLv >= 1, bMs / 1000, cLv > 0 && cLv < 1, heat, roundMeter.len);
    } else {
      const idx = roundMeter.len - beatsLeft;
      const lv = roundMeter.level(idx);                        // 1=강박 0.6=중박 0=약박
      beatKick(lv >= 1, bMs / 1000, lv > 0 && lv < 1, heat, roundMeter.len);   // 박자 길이에 맞는 그루브
...
function giftDoneToEnding() {
  ...
  // 변질된(노이즈 낀) 워시는 여기서 완전히 끄고, 대신 깨끗한 엔딩 베드를 깐다 —
  // 스코어 3초 재등장~아스키~악보~합주까지 소리가 멈추지 않게 보장한다.
  stopGiftAmbience();
  startEndingBed();
...
document.querySelectorAll('#dev-bar .devb').forEach((b) => {
  b.addEventListener('click', (e) => {
    e.stopPropagation();
    if (b.dataset.dev === 'outro') { startOutro(); return; }
    devJump(b.dataset.dev);
  });
});
...
function sendMessage() {
  ...
  // 보낸 순간의 뉘앙스(투표 리더)와 그 라운드의 박자를 함께 저장 → 엔딩 리플레이의 재료.
  state.messages.push({ player: p, pick: state.picks[p], voiceId, text, mood, garble, rhythm,
                        nuance: computeLeader() || 'period', meter: roundMeter.name });
  state.lastMood = mood;
...
let jamGroup = 'ALL';                           // 잼 그룹 차례(A/B/C/ALL) — 그 그룹 관객만 연주
let jamGroupTimers = [];                        // A→B→C→ALL 전환 타이머
let outroOn = false, outroT0 = 0;               // 아웃트로 — 관객 터치 잠금 + 40초 소리·비주얼 페이드

function startJam() {
  if (jamOn || endingPhase !== 2) return;
  jamOn = true;
  // 잼 배경은 우주 엠비언스 하나뿐(합주 진입 때부터 계속 흐른다) —
  // 관객 음표가 그 위에서 별처럼 주인공이 된다. 베드 루프는 제거(노이즈 원인).
  resumeAudio();
  ...
  jamGroupTimers.push(setTimeout(() => setGroup('ALL'), GS * 3));
}
...
function drawScore3D(ctx, W, H, t, progress) {
  ...
  // 마우스 드래그 — 구(합주·잼) 카메라를 손으로 돌린다(가로=공전, 세로=높이).
  if (sphere && (camDragYaw || camDragPitch)) {
    const cy2 = Math.cos(camDragYaw), sy2 = Math.sin(camDragYaw);
    const nx = cam.x * cy2 - cam.z * sy2, nz = cam.x * sy2 + cam.z * cy2;
    cam.x = nx; cam.z = nz;
  ...
  if (sphere) {
    ...
    ctx.fillText(jamOn
      ? (jamGroup !== 'ALL'
        ? `지금은 ${jamGroup} 그룹의 차례 — 다른 그룹은 서로의 음악에 귀 기울여요`
        : '이제 다 함께 — 핸드폰 화면을 터치해 함께 연주하세요')
  ...
  // 아웃트로 — 40초에 걸쳐 화면 전체가 검게 스러진다(소리는 fadeMasterOut이 같이 줄인다)
  if (outroOn) {
    const k = Math.min(1, (performance.now() - outroT0) / 40000);
    ctx.save();
...
function startEndingScore() {
  ...
  endingMusic = playEndingMusic(msgs);   // 타이핑 리플레이 — 박자별 묶음 합주(2배속) + 격정 비트
...
function startOrchestraPhase() {
  ...
  orchestraTracks = tracks;
  // 검정화면(합주·잼)의 소리는 우주 엠비언스 하나 + 관객 음표뿐 —
  // 합주 재생·베드류는 전부 끈다(겹치면 노이즈처럼 뭉개졌던 원인).
  stopEndingBed();
  startCosmicAmbience();
...
function stopEndingScore() {
  ...
  jamGroupTimers.forEach(clearTimeout); jamGroupTimers = [];
  jamGroup = 'ALL';
  if (outroOn) { outroOn = false; restoreMaster(); }
  document.body.classList.remove('outro-on');
  ...
  stopCosmicAmbience();   // 검정화면 우주 엠비언스 정리
...
// 그룹 배정 — 자기 파스텔 색(hsl)을 rgb로 풀어 가장 높은 채널로 A(빨강)/B(초록)/C(파랑).
// ※ tap.html 사본과 반드시 동일하게 유지.
function audGroupOf(i) {
  const h = ((i * 137.508) % 360) / 360;
  const s = AUD_SATS[((i % 4) + 4) % 4] / 100;
  const l = Math.max(30, Math.min(92, 78 + AUD_LOFF[((i % 5) + 5) % 5])) / 100;
...
function addAudienceNote(cidx, glyph) {
  ...
  if (outroOn) return;   // 아웃트로 — 관객 터치가 더는 들어오지 않는다
  // 그룹 차례 — 자기 그룹이 아닐 땐 조용히 무시(퍼포머의 익명 탭은 항상 허용)
  if (jamOn && jamGroup !== 'ALL' && Number.isInteger(cidx) && audGroupOf(cidx) !== jamGroup) return;
  const spb = 60 / SCORE_TEMPO;
  ...
  resumeAudio();
  // 사운드는 60ms에 하나만 — 100명이 동시에 눌러도 음표(시각)는 다 태어나되 오디오는 안 터진다.
  const nowMs = performance.now();
...
document.body.addEventListener('click', (e) => {
  ...
  if (act === 'qr-done') {
    // QR 화면(소개 뒤) ▶ — QR이 천천히 확대되고 곡이 고조된 뒤 대화(플레이)로.
    if (qrLeaving) return;   // 전환 중 재클릭 방지
    qrLeaving = true;
    uiClick(0.75);
    qrMusicSwell(3.5);
    const bq = $('#big-qr');
    if (bq) {
      bq.style.transition = 'transform 3.5s cubic-bezier(.4,0,.9,1), opacity 3.5s ease';
      bq.style.transform = 'scale(6)';
      bq.style.opacity = '0';
    }
    setTimeout(() => {
      qrLeaving = false;
      if (bq) { bq.style.transition = ''; bq.style.transform = ''; bq.style.opacity = ''; }
...
window.__audGroupOf = audGroupOf;   // 리허설·테스트에서 그룹 확인용
window.__probe = () => ({
