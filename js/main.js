...
const introPhotos = {};
function loadIntroPhoto(key, srcs, edge) {
  if (introPhotos[key]) return;
  const ent = introPhotos[key] = { img: new Image(), ok: false, i: 0, styled: null };
  ent.img.onload = () => { ent.ok = true; ent.styled = cartoonizePhoto(ent.img, edge); };
...
function drawIntroScene(t) {
  ...
  if (kind === 'tomato') {
    ...
    if (s < TOMATO_CUT) {
      ...
      // (사진 속 토마토들은 강한 에지로 윤곽이 도드라진다 — cartoonizePhoto edge 강화, tomatoB 전용)
...
loadIntroPhoto('tomatoB', ['assets/intro/tomato-b.jpg', 'assets/intro/tomato-b.png', 'assets/intro/tomato-b.jpeg'], 62);   // 상자 토마토 윤곽을 또렷하게(에지 낮게)
