...
const MINI_DRAW = [
  // 0 핑크토마토(핑토) — 그냥 동그란 몸 + 흰 별 꼭지 + 큰 흰 눈·세모 입
  (g, t, talking, mood) => {
    g.disc(16, 17, 10, K);
    // 흰 별 꼭지(위쪽)
    starW(g, 16, 10);
    ...
    if (blink) { g.rect(10, 17, 3, 1, W); g.rect(19, 17, 3, 1, W); }
    else { g.disc(11, 17, 1.5, W); g.disc(20, 17, 1.5, W); }
    if (talking) g.ellipse(16, 22, 2, 2, W);
    else if (mood === 'sad') { g.px(14, 23, W); g.px(15, 22, W); g.px(16, 22, W); g.px(17, 23, W); }
    else { g.rect(15, 21, 3, 1, W); g.px(16, 22, W); }
  },
  // 1 심해어 — 원래 디자인: 가로로 긴 물고기 몸 + 지느러미 + 발광체
  (g, t, talking, mood) => {
    g.ringEllipse(15, 17, 11, 6, 2, K);
    g.seg(26, 14, 30, 11, K); g.seg(26, 20, 30, 23, K); g.seg(30, 11, 30, 23, K); // 꼬리
    g.seg(12, 11, 16, 8, K); g.seg(16, 8, 19, 11, K);   // 등지느러미
    g.seg(13, 23, 16, 26, K); g.seg(16, 26, 19, 23, K); // 배지느러미
    g.seg(8, 12, 5, 6, K); g.ringDisc(5, 5, 2, 1, K);   // 발광체
    if (Math.sin(t * 3) > 0) g.px(5, 5, W);             // 깜박이는 불빛
    g.px(10, 16, K);                                     // 눈
    g.seg(4, 19, 11, 19, K);
    if (talking) g.seg(4, 20, 11, 20, K);
  },
  // 2 새 — 원래 디자인: 옆모습 몸통 + 머리 + 부리 + 날개 + 꼬리 + 가는 다리
  (g, t, talking, mood) => {
    g.ringEllipse(16, 18, 8, 6, 2, K);                   // 몸통
    g.ringDisc(10, 10, 4, 2, K);                         // 머리
    g.seg(6, 9, 2, 10, K); g.seg(6, 12, 2, 10, K); g.seg(6, 9, 6, 12, K); // 부리
    if (talking) g.seg(3, 10, 5, 11, K);
    g.px(9, 9, K);                                       // 눈
    g.seg(24, 15, 30, 13, K); g.seg(24, 21, 30, 23, K); g.seg(30, 13, 30, 23, K); // 꼬리
    g.seg(15, 15, 20, 18, K); g.seg(20, 18, 15, 20, K);  // 날개
    g.seg(14, 24, 14, 28, K); g.seg(12, 29, 16, 29, K);  // 다리
    g.seg(19, 24, 19, 28, K); g.seg(17, 29, 21, 29, K);
  },
  // 3 생쥐 — 원래 디자인: 둥근 몸 + 큰 귀 + 수염 + 긴 꼬리 + 작은 발
  (g, t, talking, mood) => {
    const tail = Math.round(Math.sin(t * 3) * 2);
    g.ringDisc(15, 17, 8, 2, K);                         // 몸
    g.ringDisc(9, 8, 3, 1, K); g.ringDisc(19, 8, 3, 1, K); // 귀
    g.px(12, 15, K); g.px(18, 15, K);                    // 눈
    g.px(15, 19, K);                                     // 코
    g.seg(8, 19, 3, 18, K); g.seg(22, 19, 27, 18, K);    // 수염
    g.seg(22, 21, 28, 19 + tail, K); g.seg(28, 19 + tail, 30, 23 + tail, K); // 꼬리
    g.seg(12, 24, 12, 27, K); g.seg(18, 24, 18, 27, K);  // 발
  },
];
