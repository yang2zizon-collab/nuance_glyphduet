...
class NoCacheHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == '/addnote':
            ...
            glyph = str(data.get('glyph') or '')[:2]   # 폰이 고른 모양(빈 값 = 메인에서 랜덤)
            cidx = None
            if uid:
        ...
        if path == '/hello':
            # 폰이 미리 자기 색 순번을 받는다(음표 없이) — 그룹·닉네임을 첫 탭 전에 알 수 있게
            data = self._body_json()
            uid = str(data.get('uid') or '')[:40]
            cidx = None
            if uid:
        ...
        if path == '/jamgroup':
            data = self._body_json()
            g = str(data.get('g') or '')
            if g not in ('A', 'B', 'C', 'ALL'):
                return self._json(400, {'ok': False, 'error': 'bad group'})
            current_group['v'] = g
            broadcast(json.dumps({'type': 'jamgroup', 'g': g}))
